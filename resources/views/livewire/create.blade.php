        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <form action="">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mb-4">
                        <label for="start_date" class="block text-gray-700 text-sm font-bold mb-2">Fecha Inicio:</label>
                    ...
                    <div class="mb-4">
                        <label for="end_date" class="block text-gray-700 text-sm font-bold mb-2">Fecha Fin:</label>
                    ...
                    <div class="mb-4">
                        <label for="id_state" class="block text-gray-700 text-sm font-bold mb-2">Estado:</label>
                        @if($name_state == "COMPLETADA")
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="id_state" id="" wire:model="id_state" disabled>
                            @foreach ($states as $state)
                            <option value="{{$state->id}}">{{$state->name}}</option>
                            @endforeach
                        </select>
                        @else
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="id_state" id="" wire:model="id_state">
                            @foreach ($states as $state)
                            <option value="{{$state->id}}">{{$state->name}}</option>
                            @endforeach
                        </select>
